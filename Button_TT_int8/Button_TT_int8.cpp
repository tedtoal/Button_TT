/*
  Button_TT_int8.cpp - Library for adding a variable of type int8_t to a button.
...
void Button_TT_int8::initButton(Adafruit_GFX *gfx, const char* align,
                  ...
                  int8_t (*checkValue)(Button_TT_int8& btn, int8_t value),
  ...
  Button_TT::initButton(gfx); // Pre-initialize base class with null values except gfx.
  ...
  gfx->setFont(f);
  Button_TT::getWidestValue(minValue, maxValue, widestValStr, W, H, showPlus);
  Button_TT::initButton(gfx, align, x, y, w, h, outlineColor, fillColor, textColor,
...
bool Button_TT_int8::setValue(int8_t value, bool dontCheck) {
  if (value < _minValue)
    value = _minValue;
  if (value > _maxValue)
    value = _maxValue;
...
bool Button_TT_int8::setValueAndDrawIfChanged(int8_t value, bool forceDraw) {
  setValue(value);
...
bool Button_TT_int8::valueIncDec(int8_t N, Button_TT_Base* btn) {
